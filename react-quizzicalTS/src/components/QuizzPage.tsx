  ...
  const submitQuizz = function(){
    quizz.forEach((eachQuizz: EachQuestionProps)=>{
      ...
      if(eachQuizz.selectedAns !== eachQuizz.answer) {
        return document.getElementsByName(eachQuizz.question).forEach((eachInput:any)=>{
 ...
 const shuffleArray = function(array: string[] | boolean[]) {
  ...
  return (
    <div className="quizz-page">
      ...
      {quizzArray}
      {!submitted && <button className="submit-btn" onClick={submitQuizz}>Check Answers</button>}

      {submitted && <div className="submitted">
       <h4>You scored {score} of {quizz.length} questions</h4>
      <button className="submit-btn play-again" onClick={playAgain}>Play Again</button> 
      </div>}
     </div>
  );
}
